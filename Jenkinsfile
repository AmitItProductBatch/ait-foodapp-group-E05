pipeline {
    ...
    // =========================================================================
    environment {
        BACKEND_IMAGE  = "${params.DOCKER_USERNAME}/food-delivery-backend-05"
        FRONTEND_IMAGE = "${params.DOCKER_USERNAME}/food-delivery-frontend-05"
        IMAGE_TAG      = "${BUILD_NUMBER}"
        VITE_API_URL   = "http://${params.SERVER_IP}:8085/api/users"
    }
    ...
    post {
        ...
        success {
            ...
            echo "👉 Frontend Application : http://${params.SERVER_IP}:4005"
            echo "👉 Backend REST API     : http://${params.SERVER_IP}:8085/api/users"
            echo "👉 PostgreSQL Database  : Port 5005 (DB: fooddelivery05)"
            echo "🎉 ========================================================"
        }
        failure {
